...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        console.log('🚀 System Settings: Starting optimized initialization...');
        
        // ⚡ Start all operations in parallel
        const [firebaseReady, configReady] = await Promise.all([
            waitForFirebaseReady().catch(error => {
                console.warn('⚠️ Firebase check failed:', error);
                return false;
            }),
            waitForSystemConfig().catch(error => {
                console.warn('⚠️ System config check failed:', error);
                return false;
            })
        ]);
        ...
        // ⚡ Check permissions and load settings in parallel (non-blocking)
        const [permissionsOK] = await Promise.all([
            checkSystemConfigPermissions().catch(error => {
                console.error('Permissions failed:', error);
                throw error; // This should still fail for security
            }),
        ...
        console.log('✅ System Settings: Parallel initialization complete');
...
// ⚡ OPTIMIZED: Fast system config check with reduced timeout
async function waitForSystemConfig() {
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            console.warn('⚠️ System config timeout (5s), proceeding anyway');
            resolve(false); // Don't reject, just warn and continue
        }, 5000); // REDUCED: 5 seconds instead of 30
